Page({
  ...
  onSubmit(e) {
    var array = ['income_avg_increase_5', 'profit_avg_increase_5'];
    var secArray = [4, 3, 2, 1];
    e.detail.value.order = array[e.detail.value.order];
    e.detail.value.quarter = secArray[e.detail.value.quarter];
    e.detail.value.desc = 1;
    if (e.detail.value.switch) {
      e.detail.value.desc = 0;
    } 
    let that = this;
    dd.httpRequest({
      url:"http://42.192.51.240:8090/stocks/profit",
      method: "POST",
      data: e.detail.value,
      dataType: 'json',
